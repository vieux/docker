...
func TestTarUntarFile(t *testing.T) {
	...
	tar, err := Tar(path.Join(origin, "before", "file"), Uncompressed)
	if err != nil {
		t.Fatal(err)
	}

	if err := Untar(tar, path.Join(origin, "after", "file"), nil); err != nil {
		t.Fatal(err)
	}

	catCmd := exec.Command("cat", path.Join(origin, "after", "file"))
